...
async function searchNostrRelays(query: string): Promise<string | undefined> {
  ...
  for (const relay of relays) {
    try {
      ...
      const result = await new Promise<string | undefined>((resolve) => {
        ...
        ws.onopen = () => {
          // NIP-50 search
          ws.send(JSON.stringify(["REQ", "search", { search: query, kinds: [0], limit: 5 }]));
        };
  ...
  for (let i = 0; i < contacts.length; i++) {
    ...
    try {
      ...
      if (result.nostrPubkey || result.npub) {
        contact.nostrPubkey = result.nostrPubkey;
        contact.npub = result.npub;
