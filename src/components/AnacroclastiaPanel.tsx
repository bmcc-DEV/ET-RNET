...
/** Gera dados simulados de artefatos */
function generateSimulatedArtifacts(): ArchaeologicalVector[] {
  return [
    { id: "art_001", omega: new Map([["cfg", 0.8], ["ssa", 0.3], ["stack", 0.5]]) },
    { id: "art_002", omega: new Map([["cfg", 0.2], ["ssa", 0.9], ["entropy", 0.7]]) },
    { id: "art_003", omega: new Map([["cfg", 0.6], ["stack", 0.4], ["depth", 0.8]]) },
    { id: "art_004", omega: new Map([["ssa", 0.5], ["entropy", 0.6], ["depth", 0.3]]) },
    { id: "art_005", omega: new Map([["cfg", 0.4], ["ssa", 0.7], ["stack", 0.9]]) },
  ];
}

/** Gera sheaf simulado */
function generateSimulatedSheaf(): CoherenceSheaf {
  const sections = new Map<string, number[]>();
  sections.set("cfg_node_0", [0.8, 0.3, 0.5]);
  sections.set("cfg_node_1", [0.2, 0.9, 0.4]);
  sections.set("cfg_node_2", [0.7, 0.1, 0.6]);
  return { sections, obstruction: 0 };
}

export default function AnacroclastiaPanel() {
  const [alpha, setAlpha] = useState(0.3);
  const artifacts = useMemo(() => generateSimulatedArtifacts(), []);
  const sheaf = useMemo(() => generateSimulatedSheaf(), []);
  ...
  /** Invariantes fósseis simulados */
  const fossilInvariants = useMemo(
    () => [
      { type: "CFG" as const, hash: "a7f3c9e2b1d84f60..." },
      { type: "SSA" as const, hash: "3e9b1d5a8c2f7406..." },
      { type: "STACK_MORPHOLOGY" as const, hash: "d4f8a2c6e1b93705..." },
    ],
    []
  );
