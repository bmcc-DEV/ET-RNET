...
import { categories, getRoutesByCategory, type RouteDef, type CategoryDef } from "../router";
          ...
          categories.map((cat) => (
            <div key={cat.id} className="mb-0.5">
              <button
                ...
                className="w-full flex items-center gap-2 px-3 py-2 text-left hover:bg-[#0a0d10] transition-colors group"
              >
                <svg className="w-3 h-3 text-zinc-600 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}>
                  <path strokeLinecap="round" strokeLinejoin="round" d={expanded[cat.id] ? "M19 9l-7 7-7-7" : "M9 5l7 7-7 7"} />
                </svg>
                <div className="w-5 h-5 flex items-center justify-center rounded" style={{ background: `${cat.color}15` }}>
                  <SvgIcon path={cat.icon} className="w-3 h-3" />
                </div>
                <span className="text-[10px] font-mono text-zinc-400 tracking-wider uppercase flex-1 group-hover:text-zinc-300">
                  {cat.label}
                </span>
                <span className="text-[8px] font-mono text-zinc-700">
                  {getRoutesByCategory(cat.id).length}
                </span>
              </button>
