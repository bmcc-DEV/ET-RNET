...
export default function ActiveTerminal() {
  ...
  useEffect(() => {
    const loadUtxos = async () => {
      ...
      // Seed some initial UTXOs if wallet is completely empty (for demo purposes)
      if (utxos.length === 0) {
  ...
  const generateGhost = async () => {
    ...
    try {
      ...
      setIsSpawning(false);
      addLog(`ERRO no GhostID Engine: ${err instanceof Error ? err.message : String(err)}`);
    }
  };
  ...
  const handlePlaceDexOrder = (e: React.FormEvent) => {
    ...
    // Generate counterparty order for matching demo
