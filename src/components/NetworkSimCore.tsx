...
export function useNetworkSimulation(nodeCount = 15) {
  ...
  useEffect(() => {
    ...
    // Source
    initialNodes.push({
      id: "src", x: 60, y: 80, type: "SOURCE", shards: ["S1", "S2", "S3"], range: 60, velocity: { x: 0, y: 0 }
    });

    // Destination
    initialNodes.push({
      id: "dst", x: 600, y: 300, type: "DEST", shards: [], range: 60, velocity: { x: 0, y: 0 }
    });

    // Gateways (LoRa)
    initialNodes.push({
      id: "gw1", x: 330, y: 150, type: "GATEWAY", shards: [], range: 200, velocity: { x: 0, y: 0 }
    });
