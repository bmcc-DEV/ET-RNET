...
/** Gera CCB simulado com tipos corretos */
function generateSimulatedCCB(): CollateralizedCollapseBond {
  return {
    id: "CCB-001",
    baseRate: 0.05,
    stressIndex: 0.3,
    klDivergence: 0.12,
    principal: 10000,
    maturityDate: Date.now() + 86400000 * 365,
    createdAt: Date.now() - 86400000 * 30,
  };
}

/** Gera HSV simulado com tipos corretos */
function generateSimulatedHSV(): HysteresisVault {
  return {
    id: "HSV-001",
    balance: 5000,
    hysteresisState: 0.85,
    lastActionTime: Date.now() - 86400000 * 7,
    beta: 0.1,
    createdAt: Date.now() - 86400000 * 90,
  };
}

/** Gera Coherence Bonds simulados com tipos corretos */
function generateSimulatedBonds(): CoherenceBond[] {
  return [
    { id: "CB-001", baseRate: 0.03, coherenceMeasure: 0.42, principal: 5000, maturityDate: Date.now() + 86400000 * 180 },
    { id: "CB-002", baseRate: 0.03, coherenceMeasure: 0.78, principal: 5000, maturityDate: Date.now() + 86400000 * 180 },
    { id: "CB-003", baseRate: 0.03, coherenceMeasure: 0.15, principal: 5000, maturityDate: Date.now() + 86400000 * 180 },
  ];
}

/** Gera Scar Tokens simulados com tipos corretos */
function generateSimulatedScars(): ScarToken[] {
  return [
    { id: "SCAR-001", defectField: [0.8, 0.3, 0.5], defectDensity: 0.53, value: 100 },
    { id: "SCAR-002", defectField: [0.2, 0.9, 0.1], defectDensity: 0.40, value: 75 },
    { id: "SCAR-003", defectField: [0.6, 0.7, 0.8], defectDensity: 0.70, value: 150 },
  ];
}
...
export default function CollapseFinancePanel() {
  ...
  const ccb = useMemo(() => generateSimulatedCCB(), []);
  const hsv = useMemo(() => generateSimulatedHSV(), []);
  const bonds = useMemo(() => generateSimulatedBonds(), []);
  const scars = useMemo(() => generateSimulatedScars(), []);
