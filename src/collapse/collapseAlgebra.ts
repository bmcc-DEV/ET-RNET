...
export class CollapseAlgebra {
  ...
  accumulate(state: CollapseState, amount: number): CollapseState {
    ...
    const perturbation = gaussianConvolve(
      Float64Array.from({ length: phi.length }, () =>
        Math.sqrt(sigma + 1) * (secureRandom() - 0.5) * 0.1
      ),
      2.0
    );
...
export function createInitialState(size: number = 64): CollapseState {
  const phi = new Float64Array(size);
  const memoryKernel = new Float64Array(size).fill(1.0);

  // Condição inicial: perturbação suave
  for (let i = 0; i < size; i++) {
    phi[i] = 0.5 * Math.sin(2 * Math.PI * i / size) + 0.1 * secureRandom();
  }
