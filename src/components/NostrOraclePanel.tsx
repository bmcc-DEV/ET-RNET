...
export default function NostrOraclePanel() {
  ...
  const handleSimulate = () => {
    const basePrice = pair === "ETR/BRL" ? 42.50 : pair === "ETR/XMR" ? 0.018 : 0.85;
    for (let i = 0; i < 5; i++) {
      const variance = (Math.random() - 0.5) * 0.1 * basePrice;
      const report: PriceReport = {
        id: `sim_${Date.now()}_${i}`,
        oraclePk: Array.from(crypto.getRandomValues(new Uint8Array(8))).map(b => b.toString(16)).join(""),
        pair: pair.toUpperCase(),
        price: basePrice + variance,
        timestamp: Date.now(),
        confidence: 0.8 + Math.random() * 0.2,
        source: ["binance", "coingecko", "kraken", "manual", "defi"][i],
      };
      nostrOracle.submitReport(report);
    }
    setStats(nostrOracle.getStats());
    addLog(`5 oraculos simulados para ${pair}`);
  };
